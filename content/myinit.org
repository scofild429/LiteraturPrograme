...
* Term mode
C-n/C-p: next/perious command(char-mode)
C-c C-j: term-line-mode(can select lines)
C-c C-k: enter term-char-mode
#+begin_src emacs-lisp
(use-package term
  :config
...
* chinese fonts
install cnfonts
M-x : cnfonts-regenerate-profile
...
** roam links
|-------+---+------------------------------|
| C-c n | f | org-roam-node-find OR *create* |
|       | i | org-roam-node-insert         |
|       | I | org-roam completion-at-point |
|       | l | backlists                    |
|-------+---+------------------------------|
  *C-c n f*  to find the  existed link or create a new one
  each roam link is a file,
  *C-c n i*  insert roam link at point
** Header links
  *org-store-link*   can be done for each header, but without any output
  *org-insert-link*   link can be chosed from history,  but this link will only show once in the history
**  Id links
*org-id-store-link*, this can add a  id link to the belonged header
*org-insert-link* chose id type,  but you have to find the id by yourself
** Journal links
only for dayly notation
|---------+---+-----------------------------|
...
* Atomic-chrome
for the bowser extension, GhostText is available for chrome and firefox
#+begin_src emacs-lisp
...
* Go
Dap-debug has to unset NODE_OPTIONS
in Terminal  unset NODE_OPTIONS
or M-x: setenv NODE_OPTIONS Enter
#+begin_src emacs-lisp
  (setq company-idle-delay 0)
  (setq company-minimum-prefix-length 1)
