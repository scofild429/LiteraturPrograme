...
| C-c C-e                  | export org                                                            |
| C-u C-space              | go back to mark postion                                               |
|--------------------------+-----------------------------------------------------------------------|
...
#+end_src
* EAF
Because of the fuck qt6-multimedia is not for Ubuntu 20
and qt5 is also not reduceable
...
| :tangle  (创建源码）   | 文件名，yes, no                          |
| :noweb yes             | 对tangle和export作用                     |
| :tangle   xxx.py       | 输出文件 (M-x: org-babel-tangle)         |
| :var  x=2              |                                          |
| :shebang               | 指定码块的解释器                         |
|                        |                                          |
|------------------------+------------------------------------------|
...
ngpg2 --output .mbsyncpass.gpg --symmetric .mbsyncpass

mu index --maildir=~/Maildir  
mu index  --maildir=~/Maildir/Gmail  --my-address=scofild429@gmail.com
...
*** latex
1- create the image through the src of lsetq org-format-latex-options (plist-put org-format-latex-options :scale 2.0))atex output with normally path
...
** org-ref
  (require 'org-ref-helm) need to install helm-bibtex
  C-c ]: insert citation
  C-Space: mark multi citations
#+begin_src emacs-lisp
...
* ebib for tex 
This package can be used in tex file for latex
M-x: ebib
o: open bib file,
e: edit
a: add
M-x: ebib-insert-citation
C-c C-c in tex file, BibTex and then XeLatex
#+begin_src 
...
#+TITLE: Guide to use orgmode for PDF file generatation
#+AUTHOR: Thomas S. Dye
