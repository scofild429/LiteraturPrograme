...
start-Schema: cons: data integrity is not gurantanted, no natural support of many to many relations
...
it based on RDDs(), which are immutable tuples
...
*** Sketch the algorithms for two-phase commit and
