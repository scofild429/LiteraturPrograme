...
| mini-batch gradient descent | only a subset of all data points in each |

iteration multi kernel calcaltion,  learing rate decay 
*** big batch size
...
Depthwise Convolution, channel-wise + 1*1 convolution
...
- advance data augmentation
  ...
  CutMit: mix patches of the two input  also target
...
- loss funtion, cross entry of pixel-wise: $\frac{1}{N} \Sum_{ij} \log p_{ij} t_{ij}$,
